...
def negSamplingLossAndGradient(
    ...
    for i in range(K): 
        ...
        loss += -np.log(y_k_hat)
        gradOutsideVecs[w_k] -= np.dot(y_k_hat - 1, centerWordVec)
        gradCenterVec -= np.dot(y_k_hat - 1, outsideVectors[w_k])
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    centorWordIdx = word2Ind[currentCenterWord]
    centerWordVec = centerWordVectors[centorWordIdx]
    for outsideWord in outsideWords:
        outsideWordIdx = word2Ind[outsideWord]
        stepLoss, gradCenter, gradOutside = word2vecLossAndGradient(centerWordVec, outsideWordIdx, outsideVectors, dataset)
        loss += stepLoss
        gradCenterVecs[centorWordIdx] += gradCenter
